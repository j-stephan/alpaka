...
{
protected:
    const alpaka::Accessor<TMemoryHandle, TBuf, TIdx, 1, alpaka::ReadAccess> mData;
    uint64_t mIndex;
...
public:
    ...
    Iterator(alpaka::Accessor<TMemoryHandle, TBuf, TIdx, 1, alpaka::ReadAccess> data, uint32_t index, uint64_t maximum)
        : mData(data)
        , mIndex(index)
        , mMaximum(maximum)
    {
...
public:
    ...
    ALPAKA_FN_HOST_ACC ALPAKA_FN_INLINE IteratorCpu(
        const TAcc& acc,
        alpaka::Accessor<TMemoryHandle, TBuf, TIdx, 1, alpaka::ReadAccess> data,
