...
namespace alpaka
{
    ...
    public:
        ...
        AtomicSycl() = default;
        //-----------------------------------------------------------------------------
        AtomicSycl(AtomicSycl const &) = default;
        //-----------------------------------------------------------------------------
    ...
    namespace traits
    {
        ...
        struct AtomicOp<AtomicAdd, AtomicSycl, T, THierarchy>
        {
            ...
            //-----------------------------------------------------------------------------
            //
            static auto atomicOp(AtomicSycl const &, T * const addr, T const & value) -> T
            {
                ...
                {
                    auto atomic_addr = cl::sycl::atomic<T, cl::sycl::access::address_space::global_space>{addr_ptr};
                    return cl::sycl::atomic_fetch_add(atomic_addr, value);
                }
            }

            template <class TInt>
            static auto simulate_atomic_add(cl::sycl::global_ptr<T> const addr, T const & value)
            {
                ...
                auto atomic_addr = cl::sycl::atomic<TInt, cl::sycl::access::address_space::global_space>{int_ptr};
                ...
                {
                    assumed = old;
                    old = cl::sycl::atomic_compare_exchange_strong(atomic_addr, assumed,
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicSub, AtomicSycl, T, THierarchy>
        {
            static_assert(std::is_integral_v<T> || std::is_floating_point_v<T>,
                          "SYCL atomics do not support this type");
            //-----------------------------------------------------------------------------
            ...
            template <class TInt>
            static auto simulate_atomic_sub(cl::sycl::global_ptr<T> const addr, T const & value)
            {
                auto raw_ptr = static_cast<T*>(addr);
                auto int_raw_ptr = reinterpret_cast<TInt*>(raw_ptr);
                auto int_ptr = cl::sycl::global_ptr<TInt>{int_raw_ptr};
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicMin, AtomicSycl, T, THierarchy>
        {
            static_assert(std::is_integral_v<T> || std::is_floating_point_v<T>,
            ...
            template <class TInt>
            static auto simulate_atomic_min(cl::sycl::global_ptr<T> const addr, T const & value)
            {
                auto raw_ptr = static_cast<T*>(addr);
                auto int_raw_ptr = reinterpret_cast<TInt*>(raw_ptr);
                auto int_ptr = cl::sycl::global_ptr<TInt>{int_raw_ptr};
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicMax, AtomicSycl, T, THierarchy>
        {
            static_assert(std::is_integral_v<T> || std::is_floating_point_v<T>,
                          "SYCL atomics don't support this type");
            //-----------------------------------------------------------------------------
            ...
            template <class TInt>
            static auto simulate_atomic_max(cl::sycl::global_ptr<T> const addr, T const & value)
            {
                auto raw_ptr = static_cast<T*>(addr);
                auto int_raw_ptr = reinterpret_cast<TInt*>(raw_ptr);
                auto int_ptr = cl::sycl::global_ptr<TInt>{int_raw_ptr};
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicExch, AtomicSycl, T, THierarchy>
        {
            static_assert(std::is_integral_v<T> || std::is_floating_point_v<T>,
                          "SYCL atomics don't support this type");
            //-----------------------------------------------------------------------------
            ...
            template <class TInt>
            static auto simulate_atomic_exchange(cl::sycl::global_ptr<T> const addr, T const & value)
            {
                auto raw_ptr = static_cast<T*>(addr);
                auto int_raw_ptr = reinterpret_cast<TInt*>(raw_ptr);
                auto int_ptr = cl::sycl::global_ptr<TInt>{int_raw_ptr};
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicInc, AtomicSycl, T, THierarchy>
        {
            static_assert(std::is_integral_v<T> || std::is_floating_point_v<T>,
                          "SYCL atomics do not support this type");
            //-----------------------------------------------------------------------------
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicDec, AtomicSycl, T, THierarchy>
        {
            static_assert(std::is_integral_v<T> || std::is_floating_point_v<T>,
                          "SYCL atomics do not support this type");
            //-----------------------------------------------------------------------------
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicAnd, AtomicSycl, T, THierarchy>
        {
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicOr, AtomicSycl, T, THierarchy>
        {
            static_assert(std::is_integral_v<T>, "Bitwise operations are not supported on non-integral types.");
            //-----------------------------------------------------------------------------
            static auto atomicOp(AtomicSycl const &, T * const addr, T const & value) -> T
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicXor, AtomicSycl, T, THierarchy>
        {
            static_assert(std::is_integral_v<T>, "Bitwise operations are not supported on non-integral types.");
            //-----------------------------------------------------------------------------
            static auto atomicOp(AtomicSycl const &, T * const addr, T const & value) -> T
        ...
        //! The SYCL accelerator atomic operation.
        template<typename T, typename THierarchy>
        struct AtomicOp<AtomicCas, AtomicSycl, T, THierarchy>
        {
            static_assert(std::is_integral_v<T> || std::is_floating_point_v<T>,
                          "SYCL atomics don't support this type");
            //-----------------------------------------------------------------------------
            static auto atomicOp(AtomicSycl const &, T * const addr, T const & compare, T const & value) -> T
            {
                auto addr_ptr = cl::sycl::global_ptr<T>{addr};
                if constexpr(std::is_integral_v<T>)
                {
                    auto atomic_addr = cl::sycl::atomic<T>{addr_ptr};
                    return cl::sycl::atomic_compare_exchange_strong(atomic_addr, compare, value);
                }
                else if constexpr(std::is_same_v<double, T>)
            ...
            template <class TInt>
            static auto simulate_atomic_compare_exchange(cl::sycl::global_ptr<T> const addr, T const & compare, T const & value)
            {
                auto raw_ptr = static_cast<T*>(addr);
                auto int_raw_ptr = reinterpret_cast<TInt*>(raw_ptr);
                auto int_ptr = cl::sycl::global_ptr<TInt>{int_raw_ptr};
