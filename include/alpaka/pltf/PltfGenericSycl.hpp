...
#pragma clang diagnostic ignored "-Wexit-time-destructors"
                static auto platform = sycl::platform{typename TPltf::selector{}};
...
#pragma clang diagnostic pop
        ...
        private:
            ...
            static auto printDeviceProperties(const sycl::device& device) -> void
            {
                ALPAKA_DEBUG_FULL_LOG_SCOPE;
                ...
                std::cout << "OpenCL profile: "
                    << device.get_info<sycl::info::device::profile>()
                    << std::endl;
                ...
                std::cout << "OpenCL C version: "
                    << device.get_info<sycl::info::device::opencl_c_version>()
                    << std::endl;

                std::cout << "Extensions: " << std::endl;
                const auto extensions = device.get_info<sycl::info::device::extensions>();
                ...
                auto item_dims = device.get_info<sycl::info::device::max_work_item_sizes>(); 
                std::cout << "(";
                ...
                auto has_img_support = device.get_info<sycl::info::device::image_support>();
                ...
                if(has_img_support)
                {
                    ...
                    std::cout << "Maximum number of 1D/2D images per image array: "
                        << device.get_info<sycl::info::device::image_max_array_size>()
                        << std::endl;
                ...
                auto print_fp_config = [](const std::string& fp,
                                          const std::vector<sycl::info::fp_config>& conf)
                {
                ...
                if(device.has_extension("cl_khr_fp16"))
                {
                    const auto fp16_conf =
                        device.get_info<sycl::info::device::half_fp_config>();
                    print_fp_config("FP16", fp16_conf);
                }

                const auto fp32_conf =
                    device.get_info<sycl::info::device::single_fp_config>();
                print_fp_config("FP32", fp32_conf);

                if(device.has_extension("cl_khr_fp64"))
                {
                    const auto fp64_conf =
                        device.get_info<sycl::info::device::double_fp_config>();
                ...
                if(has_global_mem_cache)
                {
                    ...
                    std::cout << "Global memory cache size: "
                        << device.get_info<sycl::info::device::global_mem_cache_size>() / KiB
                        << " KiB" << std::endl;
                }

                std::cout << "Global memory size: "
                    << device.get_info<sycl::info::device::global_mem_size>() / MiB
                    << " MiB" << std::endl;

                std::cout << "Maximum constant buffer allocation size: "
                    << device.get_info<sycl::info::device::max_constant_buffer_size>() / KiB
                    << " KiB" << std::endl;

                std::cout << "Maximum constant arguments per kernel: "
                    << device.get_info<sycl::info::device::max_constant_args>()
                    << std::endl;
                ...
                if(has_local_memory)
                {
                    std::cout << "Local memory size: "
                        << device.get_info<sycl::info::device::local_mem_size>() / KiB
                        << " KiB" << std::endl;
                }

                std::cout << "Error correction support: "
                    << (device.get_info<sycl::info::device::error_correction_support>() ?
                            "Yes" : "No")
                    << std::endl;
                ...
                std::cout << "Byte order: "
                    << (device.get_info<sycl::info::device::is_endian_little>() ?
                ...
                std::cout << "Queue profiling supported: "
                    << (device.get_info<sycl::info::device::queue_profiling>() ?
                            "Yes" : "No")
                    << std::endl;

                std::cout << "Built-in OpenCL kernels: " << std::endl;
                const auto builtins = device.get_info<sycl::info::device::built_in_kernels>();
                for(const auto& b : builtins)
                    std::cout << "\t" << b << std::endl;

                std::cout << "printf() buffer size: "
                    << device.get_info<sycl::info::device::printf_buffer_size>() / MiB
                    << " MiB" << std::endl;
