...
namespace alpaka
{
    ...
    namespace traits
    {
        ...
        template<>
        struct DevType<QueueSyclBlocking>
        {
            using type = DevSycl;
        };
        ...
        template<>
        struct GetDev<QueueSyclBlocking>
        {
            //-----------------------------------------------------------------------------
            ALPAKA_FN_HOST static auto getDev(QueueSyclBlocking const & queue)-> DevSycl
            {
                return queue.m_dev;
            }
        };
        ...
        template<>
        struct EventType<QueueSyclBlocking>
        {
        ...
        struct Enqueue<QueueSyclBlocking, TTask>
        {
            //-----------------------------------------------------------------------------
            ALPAKA_FN_HOST static auto enqueue(QueueSyclBlocking & queue, TTask const & task) -> void
            {
        ...
        template<>
        struct Empty<QueueSyclBlocking>
        {
            //-----------------------------------------------------------------------------
            ALPAKA_FN_HOST static auto empty(QueueSyclBlocking const & queue) -> bool
            {
        ...
        template<>
        struct CurrentThreadWaitFor<QueueSyclBlocking>
        {
            //-----------------------------------------------------------------------------
            ALPAKA_FN_HOST static auto currentThreadWaitFor(QueueSyclBlocking const & queue) -> void
            {
                ALPAKA_DEBUG_MINIMAL_LOG_SCOPE;
                // SYCL objects are reference counted, so we can just copy the queue here
                auto non_const_queue = queue;
